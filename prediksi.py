...
API_KEY = '00e2925202e588434e164ff6816543bc'
...
def save_to_db(tanggal, suhu, kelembapan, curah_hujan, lokasi, latitude, longitude):
    ...
    query = """
    INSERT INTO data_prediksi (tanggal, suhu, kelembapan, curah_hujan, lokasi, latitude, longitude)
...
# Mengambil data cuaca dan menyimpannya ke database untuk setiap kecamatan
for kecamatan, coordinates in {**kecamatan_maros, **kecamatan_pangkep}.items():
    for lat, lon in coordinates:
        weather_data = get_weather_data(lat, lon)
        if weather_data.get('cod') == 200:
            tanggal = datetime.now()
            suhu = weather_data['main']['temp']
            kelembapan = weather_data['main']['humidity']
            curah_hujan = weather_data['rain']['1h'] if 'rain' in weather_data else 0
            save_to_db(tanggal, suhu, kelembapan, curah_hujan, kecamatan, lat, lon)
